...
const Navigation = () => {
  ...
  return <header className="fixed top-0 left-0 right-0 w-full z-50 shadow-md bg-white transition-all duration-300">
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 py-2 sm:py-4 lg:py-[19px] mt-9">
        <div className="flex justify-between items-center">
          {/* Logos - Responsive sizing */}
          <div className="flex items-center gap-2 sm:gap-4">
            <img alt="GGL Logo" src="/logo.png" className="h-12 sm:h-16 lg:h-20 w-auto cursor-pointer object-contain transition-all duration-300" />
            <div className="h-6 sm:h-8 w-px bg-gray-500 hidden sm:block"></div>
            <img alt="1 Global Enterprises Logo" src="/lovable-uploads/a2513c1d-2708-4143-a69b-fa65a1d4d1f2.png" className="hidden sm:block h-8 sm:h-10 lg:h-11 w-auto object-contain transition-all duration-300" />
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center gap-4 xl:gap-6">
            <Link to={getNavLink("/home")} className={`nav-link font-medium hover:text-gc-gold text-sm xl:text-base ${isActive(getNavLink("/home")) || currentCountry.code === "SG" && isActive("/") ? "text-gc-gold" : "text-black"}`}>
              HOME
            </Link>
            <Link to={getNavLink("/about-us")} className={`nav-link font-medium hover:text-gc-gold text-sm xl:text-base ${isActive(getNavLink("/about-us")) ? "text-gc-gold" : "text-black"}`}>
              ABOUT US
            </Link>
            <Link to={getNavLink("/services")} className={`nav-link font-medium hover:text-gc-gold text-sm xl:text-base ${isActive(getNavLink("/services")) ? "text-gc-gold" : "text-black"}`}>
              SERVICES
            </Link>
            <Link to={getNavLink("/blog")} className={`nav-link font-medium hover:text-gc-gold text-sm xl:text-base ${isActive(getNavLink("/blog")) || isActive(getNavLink("/blogs")) ? "text-gc-gold" : "text-black"}`}>
              BLOGS
            </Link>
            <Link to={getNavLink("/contact")} className={`nav-link font-medium hover:text-gc-gold text-sm xl:text-base ${isActive(getNavLink("/contact")) ? "text-gc-gold" : "text-black"}`}>
              CONTACT
            </Link>
          </nav>
      ...
      {isMenuOpen && <div className="lg:hidden absolute top-full left-0 right-0 bg-white py-4 shadow-md animate-fade-in border-t max-h-[calc(100vh-80px)] overflow-y-auto">
          <div className="container mx-auto px-4">
            ...
            label: "CONTACT",
            path: "/contact"
          }].map(item => <Link key={item.path} to={getNavLink(item.path)} className={`font-medium hover:text-gc-gold py-2 text-base ${isActive(getNavLink(item.path)) || item.path === "/home" && currentCountry.code === "SG" && isActive("/") ? "text-gc-gold" : "text-black"}`} onClick={() => setIsMenuOpen(false)}>
                  {item.label}
                </Link>)}
