  ...
  const fetchGalleryImages = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('gallery').select('id, title, description, image_url, created_at').eq('country', currentCountry).or('label.is.null,label.neq.private').order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-24 pb-16">
        <div className="container mx-auto px-4">
          ...
          {/* Empty State */}
          {!loading && images.length === 0 && <div className="text-center py-16">
          ...
          {/* Gallery Grid */}
          {!loading && images.length > 0 && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            duration: 0.5,
            delay: index * 0.1
          }} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg transition-shadow duration-300 cursor-pointer" onClick={() => setSelectedImage(image)}>
                  <div className="aspect-square overflow-hidden">
                    <motion.img src={image.image_url} alt={image.title} className="w-full h-full object-cover" whileHover={{
                scale: 1.05
              }} transition={{
                duration: 0.3
              }} onError={e => {
                ...
                target.style.display = 'none';
                  ...
                  <div className="p-4">
                    ...
                    {image.description && <p className="text-sm text-gray-600 line-clamp-2">
                        {image.description}
                      </p>}
      ...
      {/* Image Modal */}
      {selectedImage && <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 z-50">
          <div className="max-w-4xl max-h-full w-full">
            <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 text-white hover:text-gray-300 text-2xl font-bold z-10">
              ×
            </button>
