...
const AirFreight = () => {
  const { country } = useParams(); // expects a route like "/:country/air-freight"
  const contactHref = country ? `/contact/${country}` : "/contact`;
  ...
  const features = [
    { icon: Clock, title: "Express Delivery", description: "Time-critical shipments with guaranteed delivery schedules" },
    { icon: Globe, title: "Global Network", description: "Extensive worldwide coverage through our partner airlines" },
    { icon: Shield, title: "Secure Handling", description: "Professional cargo handling with full insurance coverage" },
    { icon: Package, title: "Flexible Solutions", description: "Customized air freight solutions for all cargo types" }
  ];
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          ...
          >
            ...
            {/* Optional: show where it will go for clarity during dev */}
            {/* <p className="mt-3 text-sm text-black/70">Debug: {contactHref}</p> */}
          </motion.div>
        </div>
      </section>
